...
class KitchenSinkService {
    ...
    initializeToolbar() {
        ...
        this.toolbarService.create(this.commandManager, this.paperScroller,this.graph);
    ...
    saveDiagram(): void {
        console.log('Diagram saving', this.graph);

        const diagramJson = this.graph.toJSON();
        //ToDo replace with save Endpoint

        const apiUrl = 'https://your-api-endpoint.com/save-diagram';

        /*this.http.post(apiUrl, diagramJson).subscribe({
            next: (response) => {*/
        console.log('Diagram saved successfully', diagramJson);
        /*  },
          error: (error) => {
